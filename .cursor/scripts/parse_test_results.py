...
def parse_pytest_json(report_file: Path) -> Dict[str, Any]:
    ...
    for test in tests:
        if test.get("outcome") == "failed":
            ...
            failures.append({
                "nodeid": test.get("nodeid", ""),
                "file": Path(test.get("nodeid", "")).name.split("::")[0] if "::" in test.get("nodeid", "") else "",
    ...
    for filepath, file_data in files.items():
        ...
        if file_totals.get("num_statements", 0) > 0:
            file_coverage[filename] = {
                "percent": round(
                    (file_totals.get("covered_lines", 0) / file_totals.get("num_statements", 1)) * 100,
                    2
                ),
    ...
    return {
        "totals": {
            "percent_covered": round(
                ...
                2
            ) if totals.get("num_statements", 0) > 0 else 0,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse pytest JSON test results"
    )
    parser.add_argument(
        "--report",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        help="Output file for parsed results (JSON)"
    )
