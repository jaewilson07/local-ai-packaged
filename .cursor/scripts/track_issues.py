...
def find_existing_issue(
    ...
    for issue in issues:
        if (issue["test_nodeid"] == test_nodeid and 
            issue["error_message"][:200].strip() == error_key):
            return issue
    return None
...
def list_issues(status: Optional[str] = None) -> List[Dict[str, Any]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Track debugging issues from test failures"
    )
    parser.add_argument(
        "--add",
        type=str,
        help="Add issues from test results JSON file"
    )
    parser.add_argument(
        "--list",
        action="store_true",
    ...
    parser.add_argument(
        "--resolve",
        type=str,
        help="Resolve an issue by ID"
    )
    parser.add_argument(
        "--resolution",
        type=str,
        help="Resolution note (use with --resolve)"
    )
    parser.add_argument(
        "--summary",
        action="store_true",
        help="Show issue summary"
    )
    ...
    if args.add:
        ...
        if summary['recurring_issues']:
            print(f"  Recurring Issue Details:")
