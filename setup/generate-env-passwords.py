...
def generate_supabase_jwt_tokens(jwt_secret=None):
    ...
    anon_signature = hmac.new(
        ...
        anon_signed_content.encode('utf-8'),
        hashlib.sha256
    ).digest()
    ...
    service_role_signature = hmac.new(
        jwt_secret.encode('utf-8'),
        service_role_signed_content.encode('utf-8'),
        hashlib.sha256
    ).digest()
...
def main():
    ...
    if not env_path.exists():
        if example_env_path.exists():
            print(f"Creating .env from .env_sample...")
    ...
    if not lines or all(line.strip() == '' or line.strip().startswith('#') for line in lines if line.strip()):
    ...
    jwt_secret_is_placeholder = (
        not existing_jwt_secret or
        'your-' in existing_jwt_secret.lower() or
        'super-secret' in existing_jwt_secret.lower() or
        'placeholder' in existing_jwt_secret.lower() or
        'generate-' in existing_jwt_secret.lower()
    )
    
    jwt_secret = existing_jwt_secret if not jwt_secret_is_placeholder else None
    anon_key = existing_anon_key if existing_anon_key and 'your-' not in existing_anon_key.lower() else None
    service_role_key = existing_service_role_key if existing_service_role_key and 'your-' not in existing_service_role_key.lower() else None
        ...
        if '=' in stripped:
            ...
            if key == 'JWT_SECRET':
                ...
                is_placeholder = (
                    ...
                    'super-secret' in value.lower() or
                    'placeholder' in value.lower() or
                    'generate-' in value.lower()
                )
                if is_placeholder:
                ...
                if use_infisical:
                    ...
                    is_placeholder = (
                        not value or
                        'your-' in value.lower() or
                        'super-secret-key' in value.lower() or
                        'placeholder' in value.lower() or
                        'generate-' in value.lower() or
                        'example' in value.lower() or
                        value.startswith('#')  # Commented out values
                    )
                ...
                if is_placeholder:
                    ...
                    updated = True
            elif key == 'ENCRYPTION_KEY' and ('generate-' in value.lower() or 'openssl' in value.lower()):
    ...
    if jwt_keys_generated or need_jwt_secret:
        if 'JWT_SECRET' not in existing_keys_set:
            ...
            print(f"Appended missing key JWT_SECRET")
            generated_count += 1
        ...
        if need_anon_key and 'ANON_KEY' not in existing_keys_set:
            ...
            print(f"Appended missing key ANON_KEY")
            generated_count += 1
        ...
        if need_service_role_key and 'SERVICE_ROLE_KEY' not in existing_keys_set:
            ...
            print(f"Appended missing key SERVICE_ROLE_KEY")
            generated_count += 1
    ...
    if not docker_hub_username and not docker_hub_password and not docker_hub_token:
        ...
        try:
            response = input("Would you like to add Docker Hub credentials now? (y/N): ").strip().lower()
            
            if response in ['y', 'yes']:
                use_token_only = input("Use Personal Access Token only? (y/N): ").strip().lower() in ['y', 'yes']
                ...
                if use_token_only:
                    ...
                    if token:
                        ...
                        if not updated:
                            ...
                            if docker_section_idx >= 0:
                                ...
                                while insert_idx < len(new_lines) and (new_lines[insert_idx].strip().startswith('#') or new_lines[insert_idx].strip() == ''):
                                    insert_idx += 1
                    ...
                    if username:
                        password = getpass.getpass("Enter your Docker Hub password or Personal Access Token: ").strip()
                        if password:
                            ...
                            if not username_updated or not password_updated:
                                ...
                                if docker_section_idx >= 0:
                                    ...
                                    if not username_updated:
                                        new_lines.insert(insert_idx, f"DOCKER_HUB_USERNAME={username}\n")
                                        insert_idx += 1
                                    if not password_updated:
                                        new_lines.insert(insert_idx, f"DOCKER_HUB_PASSWORD={password}\n")
                                else:
                                    if not username_updated:
    ...
    if generated_count > 0:
        ...
        if not use_infisical:
            ...
            for line in current_lines:
                ...
                if stripped and '=' in stripped and not stripped.startswith('#'):
                    key = stripped.split('=', 1)[0].strip()
                    if key not in [l.split('=', 1)[0].strip() for l in merged_lines if '=' in l.strip()]:
                        merged_lines.append(line)
