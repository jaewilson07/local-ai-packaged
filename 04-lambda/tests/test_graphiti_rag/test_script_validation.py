...
async def test_validate_ai_script(mock_graphiti_rag_deps):
    ...
    with patch.object(graphiti_config, 'use_knowledge_graph', True):
        ...
        with patch("server.projects.graphiti_rag.tools.AIScriptAnalyzer") as mock_analyzer_class:
            with patch("server.projects.graphiti_rag.tools.KnowledgeGraphValidator") as mock_validator_class:
                with patch("server.projects.graphiti_rag.tools.HallucinationReporter") as mock_reporter_class:
                    # Mock analyzer
                    mock_analysis = Mock()
                    ...
                    # Mock reporter
                    mock_reporter = Mock()
                    mock_reporter.generate_comprehensive_report = Mock(return_value={
        ...
        with patch("server.projects.graphiti_rag.tools.AIScriptAnalyzer") as mock_analyzer_class:
            with patch("server.projects.graphiti_rag.tools.KnowledgeGraphValidator") as mock_validator_class:
                with patch("server.projects.graphiti_rag.tools.HallucinationReporter") as mock_reporter_class:
                    ...
                    mock_validation.hallucinations = [
                        {"type": "import", "name": "nonexistent_module", "reason": "Not found in knowledge graph"},
                        {"type": "class", "name": "NonExistentClass", "reason": "Not found in knowledge graph"}
                    ]
