...
async def test_crawl_single_page(mock_crawl4ai_deps, sample_crawl_result):
    ...
    # Mock crawler
    with patch("server.projects.crawl4ai_rag.tools.crawl_single_page", new_callable=AsyncMock) as mock_crawl:
        ...
        with patch("server.projects.crawl4ai_rag.tools.CrawledContentIngester") as mock_ingester_class:
            ...
            mock_ingester.ingest_crawled_page = AsyncMock(return_value=Mock(
                chunks_created=5,
                document_id="doc123",
                errors=[]
            ))
