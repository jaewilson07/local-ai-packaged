...
async def test_ingest_pdf(mock_mongo_client, mock_mongo_db, mock_ingestion_settings):
    ...
    pipeline = DocumentIngestionPipeline(
        config=config, documents_folder="/tmp/test_docs", clean_before_ingest=False
    )
    ...
    with patch("os.path.exists", return_value=True):
        with patch.object(
            pipeline, "_find_document_files", return_value=["/tmp/test_docs/test.pdf"]
        ):
            with patch.object(pipeline, "_read_document", return_value=("PDF content", None)):
                with patch(
                    "server.projects.mongo_rag.ingestion.pipeline.create_chunker"
                ) as mock_chunker:
                    with patch(
                        "server.projects.mongo_rag.ingestion.pipeline.create_embedder"
                    ) as mock_embedder:
                        # Mock chunker
                        mock_chunk = Mock()
                        ...
                        mock_docs_collection.insert_one = AsyncMock(
...
async def test_ingest_markdown(mock_mongo_client, mock_mongo_db, mock_ingestion_settings):
    ...
    with patch("os.path.exists", return_value=True):
        with patch.object(
            pipeline, "_find_document_files", return_value=["/tmp/test_docs/test.md"]
        ):
            with patch.object(
                pipeline, "_read_document", return_value=("# Test\n\nContent here.", None)
            ):
                with patch(
                    "server.projects.mongo_rag.ingestion.pipeline.create_chunker"
                ) as mock_chunker:
...
async def test_ingest_clean_before(mock_mongo_client, mock_mongo_db, mock_ingestion_settings):
    ...
    config = IngestionConfig(chunk_size=1000, chunk_overlap=200)
    pipeline = DocumentIngestionPipeline(
        config=config, documents_folder="/tmp/test_docs", clean_before_ingest=True
    )
    pipeline.settings = mock_ingestion_settings
    pipeline.mongo_client = mock_mongo_client
    pipeline.db = mock_mongo_db
...
async def test_ingest_error_handling(mock_mongo_client, mock_mongo_db, mock_ingestion_settings):
    ...
    with (
        ...
        patch.object(pipeline, "_find_document_files", return_value=["/tmp/test_docs/invalid.pdf"]),
        patch.object(pipeline, "_read_document", side_effect=Exception("File read error")),
    ):
        # Execute (skip initialize since we've already set up mocks)
        results = await pipeline.ingest_documents()
