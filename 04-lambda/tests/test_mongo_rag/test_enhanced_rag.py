...
async def test_query_decomposition(mock_llm_response):
    ...
    decompose_response.choices[0].message.content = "1. What is authentication?\n2. How does authentication relate to authorization?"
    
    # Mock to return different responses for each call
    mock_client.chat.completions.create = AsyncMock(side_effect=[decision_response, decompose_response])
...
async def test_result_synthesis(mock_llm_response):
    ...
    sub_query_results = [
        ...
        },
        {
            ...
            "results": [
                {
                    "content": "Authorization controls access.",
                    "similarity": 0.8
                }
            ]
        }
    ...
    mock_response.choices[0].message.content = "Authentication verifies identity, while authorization controls access to resources."
