...
from server.projects.mongo_rag.models import AgentRequest, AgentResponse
...
def test_agent_endpoint_response_quality(client, mock_agent_run, mock_agent_dependencies):
    ...
    try:
        ...
        if "authentication" in query_lower:
            assert any(term in response_lower for term in ["authentication", "verify", "identity", "user", "password", "credential"]), \
                "Response should contain relevant terms for authentication query"
    finally:
        app.dependency_overrides.clear()


def test_agent_endpoint_with_knowledge_base_query(client, mock_agent_run, mock_agent_dependencies, sample_search_results):
    ...
    try:
        ...
        assert "query" in data
        assert "response" in data
        assert len(data["response"]) > 20, "Response should be substantial when using knowledge base"
    finally:
        app.dependency_overrides.clear()
