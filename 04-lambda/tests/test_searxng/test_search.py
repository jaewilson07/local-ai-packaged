...
async def test_search_blues_muse_success(mock_searxng_response):
    ...
    # Setup
    request = SearXNGSearchRequest(query="blues muse", result_count=5)
...
async def test_search_blues_muse_with_limit(mock_searxng_response):
    ...
    # Setup
    request = SearXNGSearchRequest(query="blues muse", result_count=2)  # Limit to 2 results
...
async def test_search_blues_muse_with_engines(mock_searxng_response):
    ...
    # Setup
    request = SearXNGSearchRequest(query="blues muse", result_count=5, engines=["google", "bing"])
...
async def test_search_blues_muse_empty_query():
    ...
    # Setup
    request = SearXNGSearchRequest(query="   ", result_count=5)  # Whitespace only
