...
async def test_orchestrate_conversation_tool(mock_persona_deps):
    ...
    # Setup
    with patch('server.projects.conversation.agent.PersonaDeps.from_settings', return_value=mock_persona_deps):
        with patch('server.projects.conversation.agent.get_voice_instructions', new_callable=AsyncMock) as mock_voice:
            with patch('server.projects.conversation.agent.ConversationOrchestrator') as mock_orchestrator_class:
                with patch('server.projects.conversation.agent.record_interaction', new_callable=AsyncMock) as mock_record:
