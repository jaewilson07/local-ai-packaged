    ...
    with patch("server.api.searxng.search") as mock_searxng_search:
        ...
        mock_searxng_response = SearXNGSearchResponse(
            ...
            results=[
                ...
                for r in sample_search_results if r["engine"] in ["google", "bing"]
            ],
            count=2,
            success=True
        )
...
async def test_parse_document_html(mock_deep_research_deps, sample_fetch_response):
    ...
    request = ParseDocumentRequest(
        content=sample_fetch_response["content"],
        content_type="html"
    )
...
async def test_ingest_knowledge_success(mock_deep_research_deps, sample_parsed_chunks):
    ...
    mock_deep_research_deps.db["documents"].insert_one = AsyncMock(return_value=Mock(inserted_id=ObjectId()))
    mock_deep_research_deps.db["chunks"].insert_many = AsyncMock(return_value=Mock(inserted_ids=[ObjectId() for _ in chunks]))
...
@pytest.mark.asyncio
async def test_query_knowledge_with_graphiti(mock_deep_research_deps, sample_mongo_chunk, sample_graphiti_result):
