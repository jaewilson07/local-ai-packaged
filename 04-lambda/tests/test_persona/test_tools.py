...
async def test_get_voice_instructions(mock_persona_deps):
    ...
    mock_persona_deps.persona_store.get_conversation_context.assert_called_once_with("user1", "persona1")
...
async def test_record_interaction(mock_persona_deps):
    ...
    with patch('server.projects.persona.tools.track_mood_action', new_callable=AsyncMock) as mock_mood_action, \
         patch('server.projects.persona.tools.track_relationship_action', new_callable=AsyncMock) as mock_rel_action, \
         patch('server.projects.persona.tools.track_context_action', new_callable=AsyncMock) as mock_ctx_action:
