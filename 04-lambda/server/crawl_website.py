...
async def main():
    ...
    try:
        ...
        ingester = CrawledContentIngester(
            mongo_client=deps.mongo_client, chunk_size=1000, chunk_overlap=200
        )
        ...
        ingestion_results = await ingester.ingest_crawled_batch(
            pages=crawled_pages, chunk_size=1000, chunk_overlap=200
        )
        ...
        for i, r in enumerate(ingestion_results):
            ...
            if r.document_id:
                page_url = (
                    crawled_pages[i].get("url", "unknown") if i < len(crawled_pages) else "unknown"
                )
