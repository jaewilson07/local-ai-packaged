...
from server.projects.persona.models import MoodState, RelationshipState, ConversationContext, PersonaState, Personality
...
def generate_voice_instructions(
    ...
    if personality:
        instructions.append(f"## Your Identity")
    ...
    if mood:
        ...
        intensity_desc = "very" if mood.intensity > 0.7 else "moderately" if mood.intensity > 0.4 else "slightly"
    ...
    if relationship:
        ...
        if relationship.affection_score > 0.5:
            instructions.append(f"- You have a strong bond with this user (affection: {relationship.affection_score:.1f})")
        elif relationship.affection_score < -0.3:
            instructions.append(f"- Your relationship is strained (affection: {relationship.affection_score:.1f})")
        else:
            instructions.append(f"- You have a {relationship.affection_score:.1f} affection level with this user")
        
        if relationship.trust_level > 0.7:
            instructions.append(f"- You trust them and can be more vulnerable")
        elif relationship.trust_level < 0.4:
            instructions.append(f"- You're cautious with this user (trust: {relationship.trust_level:.1f})")
...
async def record_interaction(
    ...
    mood_result = await track_mood_action(
        user_id, persona_id, user_message, bot_response,
        deps.persona_store, deps.openai_client
    )
