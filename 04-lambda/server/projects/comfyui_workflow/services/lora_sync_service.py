...
class LoRASyncService:
    ...
    def __init__(
        ...
        google_drive_service: Optional[object] = None,
    ...
    async def ensure_lora_synced(
        ...
        try:
            ...
            file_data = await self.minio_service.download_file(
                user_id=user_id,
                object_key=f"loras/{lora_filename}"
            )
            ...
            logger.error(f"Failed to sync LoRA {lora_filename} for user {user_id}: {e}")
            return None
    ...
    async def sync_user_loras(self, user_id: UUID) -> int:
        ...
        try:
            ...
            files = await self.minio_service.list_files(
                user_id=user_id,
                prefix="loras/"
            )
            ...
            logger.error(f"Failed to sync user LoRAs for {user_id}: {e}")
            return 0
    ...
    async def sync_from_google_drive(
        ...
        try:
            ...
            logger.info(f"Downloading LoRA from Google Drive: {google_drive_file_id} for user {user_id}")
            ...
            logger.error(f"Failed to sync LoRA from Google Drive {google_drive_file_id} for user {user_id}: {e}")
            return None