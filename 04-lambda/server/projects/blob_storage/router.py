...
from server.projects.blob_storage.dependencies import get_minio_service, get_user_id
...
async def list_files(
    prefix: Optional[str] = Query(None, description="Prefix to filter files (within user folder)"),
    user: User = Depends(get_current_user),
    minio_service: MinIOService = Depends(get_minio_service)
):
    """
    ...
    **Example Usage:**
    ...
    try:
        ...
        file_metadata = [
            FileMetadata(**file_dict) for file_dict in files
        ]
...
async def download_file(
    ...
    **Example Usage:**
    ...
    try:
        ...
        logger.error(f"Failed to download file {filename} for user {user_id}: {e}")
...
async def get_file_url(
    filename: str,
    expires_in: int = Query(3600, ge=60, le=604800, description="URL expiration time in seconds (60-604800)"),
    user: User = Depends(get_current_user),
    minio_service: MinIOService = Depends(get_minio_service)
):
    """
    ...
    **Example Usage:**
    ...
    try:
        ...
        logger.error(f"Failed to generate URL for {filename} for user {user_id}: {e}")
