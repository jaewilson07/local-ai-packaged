...
def is_allowed_domain(url: str, allowed_domains: list[str] | None, base_domain: str) -> bool:
    """
    ...
    return any(domain.endswith(f".{allowed.lower()}") for allowed in allowed_domains)


def is_allowed_subdomain(url: str, allowed_subdomains: list[str] | None) -> bool:
    """
...
def extract_crawl_metadata(
    result: Any, url: str, depth: int | None = None, parent_url: str | None = None
) -> dict[str, Any]:
    """
    ...
    for depth in range(max_depth):
        ...
        async for result in await crawler.arun_many(
            ...
            if result.success and result.markdown:
                ...
                logger.debug(
                    f"📄 Streamed page {pages_processed}/{len(urls_to_crawl)}: {normalized_url}"
                )

                # Get parent URL from mapping (or use start URL for first level)
                parent_url = url_to_parent.get(
                    normalized_url, start_url_normalized if depth == 0 else None
                )
