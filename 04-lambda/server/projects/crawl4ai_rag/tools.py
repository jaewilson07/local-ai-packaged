...
from server.projects.crawl4ai_rag.services.crawler import crawl_deep, crawl_single_page
...
async def crawl_and_ingest_single_page(
    ...
    try:
        ...
        logger.info("💾 Starting storage phase: ingesting page into MongoDB/Graphiti")
        storage_start_time = datetime.now()

        ingester = CrawledContentIngester(
            mongo_client=deps.mongo_client, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )
...
async def crawl_and_ingest_deep(
    ...
    try:
        ...
        logger.info(
            f"💾 Starting storage phase: ingesting {len(crawled_pages)} pages into MongoDB/Graphiti"
        )
        storage_start_time = datetime.now()
        ...
        ingestion_results = await ingester.ingest_crawled_batch(
            pages=crawled_pages, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )
