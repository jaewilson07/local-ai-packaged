...
async def get_available_sources(mongo_client: AsyncMongoClient) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        if document_count is not None:
            await sources_collection.update_one(
                {"source_id": source_id}, {"$set": {"document_count": document_count}}
            )
        ...
        logger.exception(f"Error updating source {source_id}: {e}")


async def extract_source_summary(
    mongo_client: AsyncMongoClient, source_id: str, content: str, max_length: int = 500
) -> str:
    """
