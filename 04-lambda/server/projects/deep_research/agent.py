...
class ResearchResponse(BaseModel):
    ...
    citations: List[str] = Field(default_factory=list, description="List of citation markers [1], [2], etc.")
...
async def parse_document_tool(
    ...
    try:
        ...
        if not result.success:
            return f"Failed to parse document. Error: {result.metadata.get('error', 'Unknown error')}"
...
async def query_knowledge_tool(
    ...
    try:
        ...
        formatted = [
            f"Found {len(result.results)} relevant results for '{question}':\n"
        ]
