...
from server.projects.mongo_rag.ingestion.chunker import DoclingHybridChunker, ChunkingConfig, DocumentChunk as MongoDocumentChunk
...
async def search_web(
    ...
    """
    from server.api.searxng import search as searxng_search, SearXNGSearchRequest
    ...
    try:
        ...
        if deps.graphiti_deps and deps.graphiti_deps.graphiti:
            try:
                ...
                error_msg = f"Graphiti ingestion failed: {str(e)}"
    ...
    try:
        ...
        if request.use_graphiti and deps.graphiti_deps and deps.graphiti_deps.graphiti:
            try:
                ...
                for graph_result in graph_search_results:
                    ...
                    chunk_id = graph_result.metadata.get("chunk_id") if hasattr(graph_result, 'metadata') else None
                    document_id = graph_result.metadata.get("document_id") if hasattr(graph_result, 'metadata') else None
                    
                    # If we have chunk_id, fetch full chunk from MongoDB
                    if chunk_id:
                        from bson import ObjectId
                        try:
                            chunk_doc = await deps.db[rag_config.mongodb_collection_chunks].find_one({
                                "_id": ObjectId(chunk_id),
                                "metadata.session_id": request.session_id
                            })
                            if chunk_doc:
                                # Get document info
                                doc_info = await deps.db[rag_config.mongodb_collection_documents].find_one({
                                    "_id": chunk_doc.get("document_id")
                                })
                                
                                from server.projects.deep_research.models import CitedChunk
                                graph_results.append(CitedChunk(
                                    chunk_id=str(chunk_id),
                                    content=chunk_doc.get("content", graph_result.content if hasattr(graph_result, 'content') else ""),
                                    document_id=str(document_id) if document_id else str(chunk_doc.get("document_id", "")),
                                    document_source=doc_info.get("source", "") if doc_info else "",
                                    similarity=graph_result.similarity if hasattr(graph_result, 'similarity') else 0.8,
                                    metadata=graph_result.metadata if hasattr(graph_result, 'metadata') else {}
                                ))
                        except Exception as e:
                            ...
                            graph_results.append(CitedChunk(
                                ...
                                content=graph_result.content if hasattr(graph_result, 'content') else str(graph_result),
                                document_id=str(document_id) if document_id else "unknown",
                                document_source=graph_result.metadata.get("source", "") if hasattr(graph_result, 'metadata') else "",
                                similarity=graph_result.similarity if hasattr(graph_result, 'similarity') else 0.8,
                                metadata=graph_result.metadata if hasattr(graph_result, 'metadata') else {}
                            ))
                    else:
        ...
        if request.search_type == "graph" and graph_results:
            return QueryKnowledgeResponse(
                results=graph_results[:request.match_count],
        ...
        if request.search_type in ["semantic", "hybrid"]:
            ...
            vector_search_stage = {
                "$vectorSearch": {
                    ...
                    "path": "embedding",
                    "numCandidates": 100,
                    "limit": request.match_count * 2 if request.search_type == "hybrid" else request.match_count,
                    "filter": filter_dict
                }
            }
