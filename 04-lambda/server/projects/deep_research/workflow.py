...
from server.projects.deep_research.agent import linear_researcher_agent, ResearchResponse
from server.projects.deep_research.dependencies import DeepResearchDeps

logger = logging.getLogger(__name__)


async def run_linear_research(
    query: str,
    ...
    try:
        ...
        result = await linear_researcher_agent.run(
            query,
            deps=deps
        )
        ...
        logger.exception(f"Error running linear research: {e}")
        raise
