...
from server.projects.deep_research.state import ResearchState, ResearchVector, Citation
from server.projects.deep_research.dependencies import DeepResearchDeps
from server.projects.deep_research.tools import (
    search_web, fetch_page, parse_document, ingest_knowledge, query_knowledge
)
from server.projects.deep_research.models import (
    SearchWebRequest, FetchPageRequest, ParseDocumentRequest,
    IngestKnowledgeRequest, QueryKnowledgeRequest, DocumentChunk
)
...
_current_deps: Optional[DeepResearchDeps] = None
...
async def planner_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        pre_search_request = SearchWebRequest(
...
"""
        ...
        result = await planner_agent.run(planner_prompt, deps=deps)
...
async def executor_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        # Parse
        parse_request = ParseDocumentRequest(
        ...
        logger.exception(f"Error in executor node: {e}")
        vector.status = "failed"
        state['errors'].append(f"Executor error for {vector.id}: {str(e)}")
        state['current_vector_index'] += 1
        return state
...
"""
        ...
        result = await auditor_agent.run(auditor_prompt, deps=deps)
        ...
        if len(query_response.results) >= 2:
            ...
            if vector.feedback_loop_count < 3:  # Max 3 refinement attempts
                ...
                vector.status = "failed"
                logger.error(f"Vector {vector.id} failed after {vector.feedback_loop_count} attempts")
        
        state['current_vector_index'] += 1
        return state
        
    except Exception as e:
        logger.exception(f"Error in auditor node: {e}")
        vector.status = "failed"
        state['errors'].append(f"Auditor error for {vector.id}: {str(e)}")
        state['current_vector_index'] += 1
        return state
...
async def writer_node(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        for i, section_title in enumerate(state['outline']):
            ...
            chunks_text = "\n\n".join([
                f"[{i+1}] {chunk.content}" 
                for i, chunk in enumerate(query_response.results)
            ])
...
"""
        ...
        logger.exception(f"Error in writer node: {e}")
