...
class SearchResult(BaseModel):
    ...
    engine: Optional[str] = Field(None, description="Search engine that provided this result")
...
class SearchWebRequest(BaseModel):
    ...
    engines: Optional[List[str]] = Field(None, description="Optional engine filters")
    result_count: int = Field(default=5, ge=1, le=20, description="Number of results to return (1-20)")
...
class FetchPageResponse(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Page metadata (title, description, etc.)")
...
class DocumentChunk(BaseModel):
    ...
    token_count: Optional[int] = Field(None, description="Token count for this chunk")
...
class ParseDocumentRequest(BaseModel):
    ...
    content_type: Literal["html", "markdown", "text"] = Field(
        default="html",
        description="Content type hint for parsing"
    )
