...
class ConversationOrchestrator:
    ...
    def __init__(self, llm_client: Optional[openai.AsyncOpenAI] = None):
        """
...
Example: search_knowledge|enhanced_search|Search knowledge base and provide answer"""
            ...
            result = response.choices[0].message.content or "respond_directly||Provide a helpful response"
            parts = result.split("|")
    ...
    async def generate_response(
        ...
        try:
            ...
            if plan:
                prompt_parts.append("")
                prompt_parts.append(f"Response strategy: {plan.get('strategy', 'Provide a helpful response')}")
            ...
            prompt_parts.append("")
            prompt_parts.append("Generate a helpful, natural response based on the persona context and any tool results.")
