...
async def view_storage_data(
    prefix: Optional[str] = Query(None, description="Prefix to filter files (e.g., 'loras/')"),
    ...
    **Example Usage:**
    ...
    try:
        if is_admin:
            ...
            for obj in objects:
                ...
                if len(key_parts) > 0 and key_parts[0].startswith('user-'):
                    user_uuid = key_parts[0][5:]  # Remove 'user-' prefix
        ...
        logger.exception(f"Failed to list storage files: {e}")
...
async def view_supabase_data(
    ...
    **Example Usage:**
    ...
    try:
        ...
        logger.exception(f"Failed to fetch Supabase data: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch data: {str(e)}")


@router.get("/neo4j", response_model=Neo4jDataResponse)
async def view_neo4j_data(
    node_type: Optional[str] = Query(None, description="Filter by node type/label (e.g., 'Document')"),
    user: User = Depends(get_current_user),
    auth_service: AuthService = Depends(get_auth_service),
    ...
    **Example Usage:**
    ...
    try:
        async with driver.session() as session:
            ...
            for record in records:
                ...
                if node and node.id not in seen_node_ids:
                    ...
                    nodes.append(Neo4jNodeResponse(
                        id=str(node.id),
                ...
                if rel and rel.id not in seen_rel_ids:
                    ...
                    relationships.append(Neo4jRelationshipResponse(
                        id=str(rel.id),
        ...
        logger.exception(f"Failed to fetch Neo4j data: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch data: {str(e)}")
    ...
    **Example Usage:**
    ...
    try:
        ...
        for doc in documents:
            ...
            formatted_docs.append(MongoDBDocumentResponse(
                id=doc_id,
