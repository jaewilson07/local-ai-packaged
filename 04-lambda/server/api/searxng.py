...
async def search(request: SearXNGSearchRequest):
    ...
    **Integration:**
    ...
    try:
        ...
        logger.exception(f"searxng_timeout: {request.query}")
        raise HTTPException(
            status_code=504,
            detail="SearXNG request timed out. The search engine may be overloaded.",
        )
    except httpx.HTTPStatusError as e:
        logger.exception(f"searxng_http_error: {e.response.status_code} - {request.query}")
        raise HTTPException(
            status_code=502, detail=f"SearXNG returned error: {e.response.status_code}"
        )
    except httpx.RequestError as e:
        logger.exception(f"searxng_connection_error: {e} - {request.query}")
        raise HTTPException(
            status_code=503,
        ...
        raise HTTPException(status_code=500, detail=f"Unexpected error during search: {e!s}")
