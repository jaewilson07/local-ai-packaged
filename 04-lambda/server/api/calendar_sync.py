...
class CalendarSyncRequest(BaseModel):
    ...
    event_name_pattern: Optional[str] = Field(None, description="Optional pattern or name to use for the event")
    ...
    location_pattern: Optional[str] = Field(None, description="Optional location pattern or specific location string")
    description_template: Optional[str] = Field(None, description="Optional description template or specific description")
    workflow_name: str = Field("Scrape Event To Calendar", description="Name of the n8n workflow to use")
...
class CalendarSyncResponse(BaseModel):
    ...
    start: Optional[str] = Field(None, description="Event start date/time")
    end: Optional[str] = Field(None, description="Event end date/time")
    ...
    try:
        ...
        if not content_found:
            ...
            try:
                ...
                logger.info(f"Crawled and ingested {request.url} into RAG. Chunks: {crawl_result.chunks_created}")
            except Exception as e:
        ...
        logger.exception(f"calendar_sync_error: {e}")
