...
class SupabaseService:
    ...
    async def _ensure_profiles_table(self) -> None:
        ...
        async with pool.acquire() as conn:
            ...
            if not table_exists:
                ...
                # Check if immich_user_id and immich_api_key columns exist, add if not
                ...
                # Check if discord_user_id column exists, add if not
    ...
    async def get_user_by_email(self, email: str) -> User | None:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                SELECT id, email, role, tier, created_at, mongodb_username, mongodb_password, immich_user_id, immich_api_key, discord_user_id
                FROM profiles
                WHERE email = $1
            """,
                email,
    ...
    async def create_user(
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute(
                """
                INSERT INTO profiles (id, email, role, tier, created_at, updated_at, mongodb_username, mongodb_password, immich_user_id, immich_api_key)
            ...
            # Store MongoDB and Immich credentials in user object
            if hasattr(user, "__dict__"):
    ...
    async def get_immich_credentials(self, email: str) -> tuple[str | None, str | None]:
        ...
        Returns:
            Tuple of (immich_user_id, immich_api_key) or (None, None) if not found
        """
        pool = await self._get_pool()
    ...
    async def get_user_by_discord_id(self, discord_user_id: str) -> User | None:
        ...
        async with pool.acquire() as conn:
            ...
            if row:
                ...
                # Store credentials in user object if available
                if hasattr(user, "__dict__"):
                    user.__dict__["mongodb_username"] = row.get("mongodb_username")
                    user.__dict__["mongodb_password"] = row.get("mongodb_password")
                    user.__dict__["immich_user_id"] = row.get("immich_user_id")
