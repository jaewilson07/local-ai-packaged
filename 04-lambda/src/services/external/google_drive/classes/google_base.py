...
class GoogleBaseSearch(ABC):
    ...
    def search(self, query: str, top_n: int = 10, **kwargs) -> Any:
        ...
        results = self._execute_search(query, page_size=top_n, **kwargs)
...
class GoogleBaseExport(ABC):
    ...
    def export_as_markdown(self, file_id: str, include_metadata: bool = True, **kwargs) -> str:
        """Export content as markdown with optional metadata.
        ...
        Returns:
            Markdown content with optional frontmatter
        """
        # Get file metadata
        metadata = self._parent.get_file_metadata(file_id)
        mime_type = metadata["mimeType"]

        # Download content
        content = self._download_content(file_id, mime_type)
