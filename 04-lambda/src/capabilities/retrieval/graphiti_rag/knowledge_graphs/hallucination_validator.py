...
class KnowledgeGraphValidator:
    ...
    async def _validate_single_attribute_access(
        ...
        if not attr_info:
            ...
            if method_info:
                ...
                return AttributeValidation(
                    attribute_access=attr_access, validation=validation, expected_type="method"
                )
        ...
        return AttributeValidation(
            attribute_access=attr_access, validation=validation, expected_type=attr_info.get("type")
        )
    ...
    def _validate_parameters(
        self, expected_params: list[str], provided_args: list[str], provided_kwargs: dict[str, str]
    ) -> ValidationResult:
        ...
        return ValidationResult(
            status=ValidationStatus.VALID, confidence=0.9, message="Parameters are valid"
        )
        ...
        async with self.driver.session() as session:
            ...
            if "." in class_name:
                ...
                if repo_name:
                    ...
                    result = await session.run(
                        repo_query, repo_name=repo_name, class_name=class_part, attr_name=attr_name
                    )
                    record = await result.single()
    ...
    def _calculate_overall_confidence(self, result: ScriptValidationResult) -> float:
        ...
        for val in result.class_validations:
            class_name = (
                val.class_instantiation.full_class_name or val.class_instantiation.class_name
            )
