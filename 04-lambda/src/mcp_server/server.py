...
async def agent_query(query: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Agent query failed: {e.detail}") from e
...
async def get_available_sources() -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to get sources: {e.detail}") from e
...
async def parse_github_repository(repo_url: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Parse failed: {e.detail}") from e
...
async def check_ai_script_hallucinations(script_path: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Validation failed: {e.detail}") from e
...
async def query_knowledge_graph(command: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Query failed: {e.detail}") from e
...
async def crawl_single_page(
    ...
    including title, description, language, images, and links. Supports authentication
    via cookies and headers for protected pages.
    ...
    Args:
        ...
        cookies: Optional authentication cookies as string (e.g., "sessionid=abc123; csrftoken=xyz")
                or dict. Cookies are automatically set for the page's domain.
        headers: Optional custom HTTP headers as dict (e.g., {"Authorization": "Bearer token"}).
...
async def crawl_deep(
    ...
    and parent URL for traceability. Supports authentication via cookies and headers
    for protected pages.
    ...
    Args:
        ...
        cookies: Optional authentication cookies as string (e.g., "sessionid=abc123; csrftoken=xyz")
                or dict. Cookies are automatically set for the starting URL's domain and maintained
                across all crawled pages.
        headers: Optional custom HTTP headers as dict (e.g., {"Authorization": "Bearer token"}).
                Headers are included in all requests during the crawl.
...
@mcp.tool
async def download_page_markdown(
    url: str,
    ...
    """
    Download a single web page as markdown.

    Crawls the page and returns markdown content. Optionally saves to file.
    Does not ingest into MongoDB RAG. Supports authentication via cookies and headers.

    Args:
        url: URL to download. Must be a valid HTTP/HTTPS URL.
        save_to_file: Whether to save markdown to a file. Default: False.
        output_path: Optional output path. If directory, generates filename from URL.
                    If file path, uses that path. If None and save_to_file=True, saves to current directory.
        cookies: Optional authentication cookies as string (e.g., "sessionid=abc123; csrftoken=xyz")
                or as dict. Cookies are automatically set for the page's domain.
        headers: Optional custom HTTP headers as dict (e.g., {"Authorization": "Bearer token"}).
                Headers are included in all requests.
    ...
    Returns:
        ...
        - url: str - The downloaded URL
        - markdown: str - The markdown content
        - metadata: dict - Page metadata (title, description, images, etc.)
        - file_path: str | None - Path to saved file if save_to_file=True
        - error: str | None - Error message if success=False
    """
    from workflows.ingestion.crawl4ai_rag.dependencies import Crawl4AIDependencies
    from workflows.ingestion.crawl4ai_rag.services.downloader import download_page_as_markdown

    deps = Crawl4AIDependencies.from_settings()
    await deps.initialize()
    try:
        result = await download_page_as_markdown(
            deps.crawler, url, save_to_file, output_path, cookies, headers
        )
        return result
    except Exception as e:
        logger.exception("Error in download_page_markdown")
...
@mcp.tool
async def download_website_markdown(
    url: str,
    max_depth: int,
    allowed_domains: list[str] | None = None,
    allowed_subdomains: list[str] | None = None,
    ...
    """
    Download multiple web pages as markdown (deep crawl).

    Recursively crawls a website and returns markdown content for all discovered pages.
    Optionally saves to files. Does not ingest into MongoDB RAG.
    Supports authentication via cookies and headers.
    ...
    Args:
        ...
        allowed_subdomains: List of allowed subdomain prefixes. If not provided, allows all
                           subdomains. Example: ["docs", "api", "blog"] matches docs.example.com,
                           api.example.com, etc.
        save_to_files: Whether to save markdown to files. Default: False.
        output_directory: Optional output directory for files. If None and save_to_files=True,
                         saves to current directory. Each page gets a unique filename based on its URL.
        cookies: Optional authentication cookies as string (e.g., "sessionid=abc123; csrftoken=xyz")
                or as dict. Cookies are automatically set for the starting URL's domain and maintained
                across all crawled pages.
        headers: Optional custom HTTP headers as dict (e.g., {"Authorization": "Bearer token"}).
                Headers are included in all requests during the crawl.
    ...
    Returns:
        ...
        - pages: list[dict] - Array of page data, each with url, markdown, metadata, and optionally file_path
        - total_pages: int - Total number of pages downloaded
        - file_paths: list[str] | None - List of file paths if save_to_files=True
        - error: str | None - Error message if success=False
    """
    from workflows.ingestion.crawl4ai_rag.dependencies import Crawl4AIDependencies
    from workflows.ingestion.crawl4ai_rag.services.downloader import download_pages_as_markdown
    ...
    try:
        ...
        logger.exception("Error in download_website_markdown")
        return {"error": str(e), "success": False, "url": url, "pages": [], "total_pages": 0}
    finally:
        await deps.cleanup()
...
async def web_search(
    ...
    try:
        ...
        raise RuntimeError(f"Web search failed: {e.detail}") from e
...
async def scrape_event_to_calendar(
    ...
    try:
        ...
        raise RuntimeError(f"Calendar sync failed: {error_detail}") from e
...
async def discover_n8n_nodes(category: str | None = None) -> str:
    ...
    from workflows.automation.n8n_workflow.tools import discover_n8n_nodes
...
async def search_n8n_knowledge_base(
    ...
    from pydantic_ai import RunContext
    from workflows.automation.n8n_workflow.dependencies import N8nWorkflowDeps
    from workflows.automation.n8n_workflow.tools import search_n8n_knowledge_base
...
async def search_n8n_node_examples(
    ...
    from pydantic_ai import RunContext
    from workflows.automation.n8n_workflow.dependencies import N8nWorkflowDeps
    from workflows.automation.n8n_workflow.tools import search_node_examples
...
async def extract_events_from_content(
    ...
    try:
        ...
        raise RuntimeError(f"Event extraction failed: {e.detail}") from e
...
async def fetch_page(url: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Fetch failed: {e.detail}") from e
...
async def ingest_youtube_video(
    ...
    Extracts the video transcript, metadata, and optionally chapters, entities, and topics.
    The video becomes immediately searchable via search endpoints. Supports authentication
    via cookies for age-restricted or member-only videos.
    ...
    Args:
        ...
        extract_chapters: Extract chapter markers from video. Default: True.
        extract_entities: Use LLM to extract named entities (people, products, concepts).
                         Slower but provides richer metadata. Default: False.
        extract_topics: Use LLM to classify video into topic categories. Default: False.
        extract_key_moments: Use LLM to identify key moments with timestamps. Default: False.
        chunk_by_chapters: If chapters are available, chunk transcript by chapter boundaries
                          for better semantic grouping. Default: True.
        chunk_size: Chunk size for document splitting when not using chapters.
                   Range: 100-5000. Default: 1000.
        chunk_overlap: Chunk overlap size. Range: 0-500. Default: 200.
        preferred_language: Preferred transcript language code (e.g., 'en', 'es', 'ja').
    ...
    Returns:
        ...
        - transcript_language: str - Language of transcript used
        - chapters_found: int - Number of chapters found
        - entities_extracted: int - Number of entities extracted (if enabled)
        - topics_classified: list - Classified topics (if enabled)
    ...
    try:
        ...
        try:
            request = IngestYouTubeRequest(
                ...
                chunk_by_chapters=chunk_by_chapters,
                chunk_size=chunk_size,
                chunk_overlap=chunk_overlap,
                preferred_language=preferred_language,
            )
            result = await ingest_tool(deps, request)
            return result.dict()
        finally:
            await deps.cleanup()
    except ValidationError as e:
        logger.warning("mcp_validation_error: ingest_youtube_video", extra={"errors": e.errors()})
        raise ValueError(f"Invalid parameters: {e}") from e
...
async def get_youtube_metadata(
    ...
    """
    Get YouTube video metadata without ingesting into the knowledge base.

    Useful for previewing video content before deciding to ingest, or for
    getting quick information about a video.

    Args:
        url: YouTube video URL.
        include_transcript: Whether to also fetch the transcript. Default: False.

    Returns:
        Dictionary containing:
        - success: bool - Whether retrieval succeeded
        - url: str - Original YouTube URL
        - metadata: dict - Video metadata (title, channel, duration, views, etc.)
        - transcript: dict | None - Transcript data if requested
    ...
    try:
        ...
        logger.warning("mcp_validation_error: get_youtube_metadata", extra={"errors": e.errors()})
        raise ValueError(f"Invalid parameters: {e}") from e
...
async def search_youtube_videos(
    ...
    """
    Search for YouTube videos in the knowledge base.

    Searches across all ingested YouTube videos using the specified search type.
    Results are filtered to only include YouTube content.
    ...
    Args:
        ...
        search_type: Type of search to perform:
                    ...
                    - "hybrid": Combines both (recommended). Default: "hybrid".
        channel_filter: Optional filter to only return videos from specific channel.
    ...
    try:
        ...
        metadata_filter = {"source_type": "youtube"}
        if channel_filter:
            metadata_filter["channel_name"] = channel_filter
        ...
        logger.warning("mcp_validation_error: search_youtube_videos", extra={"errors": e.errors()})
        raise ValueError(f"Invalid parameters: {e}") from e
    except HTTPException as e:
        logger.warning(
            "mcp_http_error: search_youtube_videos",
            extra={"status_code": e.status_code, "detail": e.detail},
        )
        raise RuntimeError(f"Search failed: {e.detail}") from e
