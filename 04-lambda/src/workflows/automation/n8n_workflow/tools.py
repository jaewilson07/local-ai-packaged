...
async def create_workflow(
    ...
    try:
        ...
        logger.info(
            "workflow_created", extra={"workflow_id": workflow_id, "name": name, "active": active}
        )
        ...
        logger.exception("workflow_creation_failed", extra={"error": error_msg})
        return error_msg
    except Exception as e:
        error_msg = f"Error creating workflow: {e!s}"
...
async def update_workflow(
    ...
    try:
        ...
        logger.exception("workflow_not_found", extra={"workflow_id": workflow_id})
        return error_msg
    ...
    try:
        ...
        logger.exception(
            "workflow_update_failed", extra={"workflow_id": workflow_id, "error": error_msg}
        )
        return error_msg
    except Exception as e:
        error_msg = f"Error updating workflow: {e!s}"
        logger.exception("workflow_update_error")
        return error_msg


async def delete_workflow(ctx: RunContext[N8nWorkflowDeps], workflow_id: str) -> str:
    """
    ...
    try:
        ...
        logger.exception(
            "workflow_deletion_failed", extra={"workflow_id": workflow_id, "error": error_msg}
        )
        return error_msg
    except Exception as e:
        error_msg = f"Error deleting workflow: {e!s}"
        logger.exception("workflow_deletion_error")
        return error_msg


async def activate_workflow(
    ctx: RunContext[N8nWorkflowDeps], workflow_id: str, active: bool
) -> str:
    """
    ...
    try:
        ...
        logger.info(
            "workflow_activation_changed", extra={"workflow_id": workflow_id, "active": active}
        )
        ...
        if e.response.status_code == 404:
            return f"Workflow {workflow_id} not found"
        error_msg = (
            f"Failed to change workflow activation: {e.response.status_code} - {e.response.text}"
        )
        logger.exception(
            "workflow_activation_failed", extra={"workflow_id": workflow_id, "error": error_msg}
        )
        return error_msg
    except Exception as e:
        error_msg = f"Error changing workflow activation: {e!s}"
        logger.exception("workflow_activation_error")
        return error_msg


async def list_workflows(ctx: RunContext[N8nWorkflowDeps], active_only: bool = False) -> str:
    """
    ...
    try:
        ...
        logger.exception("workflow_list_failed", extra={"error": error_msg})
        return error_msg
    except Exception as e:
        error_msg = f"Error listing workflows: {e!s}"
        logger.exception("workflow_list_error")
        return error_msg


async def execute_workflow(
    ctx: RunContext[N8nWorkflowDeps], workflow_id: str, input_data: dict[str, Any] | None = None
) -> str:
    """
    ...
    try:
        ...
        response = await deps.http_client.post(
            f"/workflows/{workflow_id}/execute", json=execution_data
        )
        response.raise_for_status()
        result = response.json()
        ...
        logger.info(
            "workflow_executed", extra={"workflow_id": workflow_id, "execution_id": execution_id}
        )
        ...
        if e.response.status_code == 404:
            ...
            try:
                ...
                webhook_node = next(
                    (n for n in nodes if n.get("type") == "n8n-nodes-base.webhook"), None
                )
                ...
                return f"Failed to check workflow execution method: {inner_e!s}"

        error_msg = f"Failed to execute workflow: {e.response.status_code} - {e.response.text}"
        logger.exception(
            "workflow_execution_failed", extra={"workflow_id": workflow_id, "error": error_msg}
        )
        return error_msg
    except Exception as e:
        error_msg = f"Error executing workflow: {e!s}"
        logger.exception("workflow_execution_error")
        return error_msg


async def discover_n8n_nodes(ctx: RunContext[N8nWorkflowDeps], category: str | None = None) -> str:
    """
    ...
    try:
        ...
        if category:
            ...
            nodes = [
                n
                for n in nodes
                if category_lower in str(n.get("category", "")).lower()
                or category_lower in str(n.get("type", "")).lower()
            ]
        ...
        logger.exception("node_discovery_failed", extra={"error": error_msg})
        return f"{error_msg}. Try using search_n8n_knowledge_base to find node information."
    except Exception as e:
        error_msg = f"Error discovering nodes: {e!s}"
...
async def search_n8n_knowledge_base(
    ...
    try:
        ...
        logger.info(
            "n8n_knowledge_search_completed", extra={"query": query, "results": len(results)}
        )
...
async def search_node_examples(
    ...
    try:
        ...
        results = await hybrid_search(ctx=deps_ctx, query=search_query, match_count=match_count)
        ...
        logger.info(
            "node_examples_search_completed", extra={"query": search_query, "results": len(results)}
        )

        return "\n".join(response_parts)

    except Exception as e:
        error_msg = f"Error searching node examples: {e!s}"
