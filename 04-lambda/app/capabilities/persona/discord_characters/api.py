...
from app.capabilities.persona.discord_characters.dependencies import DiscordCharactersDeps
from app.capabilities.persona.discord_characters.models import (
    AddCharacterRequest,
    CharacterResponse,
    ChatRequest,
    ChatResponse,
...
from app.capabilities.persona.persona_state.dependencies import PersonaDeps
    ...
    try:
        ...
        try:
            ...
            success, message = await deps.character_manager.add_character(
                request.channel_id, request.character_id, request.persona_id or request.character_id
            )
        ...
        logger.exception("Error adding character")
...
async def remove_character_endpoint(
    ...
    try:
        success, message = await deps.character_manager.remove_character(
            request.channel_id, request.character_id
        )
        ...
        logger.exception("Error removing character")
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/list", response_model=list[CharacterResponse])
async def list_characters_endpoint(
    channel_id: str, deps: Annotated[DiscordCharactersDeps, Depends(get_discord_characters_deps)]
):
...
async def engage_endpoint(
    ...
    try:
        ...
        if not should_engage:
            return EngageResponse(should_engage=False, character_id=request.character_id)
        ...
        try:
            ...
            plan = await orchestrator.plan_response(engagement_prompt, voice_instructions, [])
            ...
            return EngageResponse(
                should_engage=True, response=response, character_id=request.character_id
            )
        finally:
            await persona_deps.cleanup()
