...
from app.workflows.research.deep_research.dependencies import DeepResearchDeps
from app.workflows.research.deep_research.orchestrator import get_research_graph
from app.workflows.research.deep_research.state import ResearchState

logger = logging.getLogger(__name__)


async def run_storm_research(
    query: str, session_id: str | None = None, max_iterations: int = 10
) -> ResearchState:
    """
    ...
    try:
        ...
        logger.info(
            f"STORM research completed. Report length: {len(final_state.get('final_report', '') or '')}"
        )
        return final_state

    except Exception as e:
        logger.exception("Error running STORM research")
        initial_state["errors"].append(str(e))
        initial_state["final_report"] = f"Error: {e!s}"
