...
async def call_mcp_tool(tool_name: str, arguments: dict[str, Any]) -> Any:
    ...
    try:
        ...
        if aiohttp and isinstance(http_error, aiohttp.ClientResponseError):
            e = http_error
            logger.exception(f"MCP tool call HTTP error: {e.status} - {e.message}")
            raise ConnectionError(f"MCP server returned error {e.status}: {e.message}")
        if (httpx and isinstance(http_error, httpx.RequestError)) or (
            aiohttp and isinstance(http_error, aiohttp.ClientError)
        ):
            e = http_error
            logger.exception(f"MCP tool call connection error: {e}")
            raise ConnectionError(f"Unable to connect to MCP server at {url}: {e}")
    except json.JSONDecodeError as e:
        logger.exception("MCP tool call JSON decode error")
        raise RuntimeError(f"Invalid JSON response from MCP server: {e}")
    except Exception:
        logger.exception("Unexpected error calling MCP tool '{tool_name}'")
        raise
