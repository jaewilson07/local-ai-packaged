...
to compose/supabase/docker-compose.yml, preserving network configuration and volume path adjustments.
"""
...
def update_supabase_compose():
    """Update compose/supabase/docker-compose.yml from upstream Supabase files."""
    
    source_main = "supabase/docker/docker-compose.yml"
    source_s3 = "supabase/docker/docker-compose.s3.yml"
    target = "compose/supabase/docker-compose.yml"
    
    if not os.path.exists(source_main):
        print(f"Error: Source file not found: {source_main}")
        print("Please ensure Supabase repository is cloned in the supabase/ directory.")
        sys.exit(1)
...
"""
    ...
    # Update volume paths to be relative to compose/supabase/
    # Replace ./volumes/ with ../../supabase/docker/volumes/
    content = content.replace("./volumes/", "../../supabase/docker/volumes/")
    ...
    print("  1. Network configuration is correct (external: localai_default)")
    print("  2. Volume paths are relative to compose/supabase/")
