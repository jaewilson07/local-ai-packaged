...
def query_neo4j_cypher(cypher_query: str) -> dict[str, Any]:
    ...
    payload = {"command": f"query {cypher_query}"}
...
def explore_graphiti_data():
    ...
    query_neo4j_cypher(
        "MATCH (n) RETURN DISTINCT labels(n) as node_type, count(n) as count ORDER BY count DESC LIMIT 20"
    )
    ...
    print("\n📝 Example 5: Find Facts About Blues Muse")
    query_neo4j_cypher(
        "MATCH (f:Fact) WHERE f.fact CONTAINS 'Blues Muse' OR f.fact CONTAINS 'blues' RETURN f.fact as fact LIMIT 10"
    )

    # Example 6: View entity relationships
    print("\n📝 Example 6: View Entity Relationships")
    query_neo4j_cypher(
        "MATCH (e1:Entity)-[r:RELATES_TO]->(e2:Entity) RETURN e1.name as from, type(r) as relationship, e2.name as to LIMIT 10"
    )
