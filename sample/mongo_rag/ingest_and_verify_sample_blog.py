...
# Configuration
LAMBDA_API_BASE = os.getenv("LAMBDA_API_BASE", "http://localhost:8000")
API_BASE_URL = f"{LAMBDA_API_BASE}/api/v1/rag"
...
async def ingest_document(file_path: Path, clean_before: bool = False) -> Dict[str, Any]:
    ...
    files = {
        "files": (file_path.name, file_content, "text/markdown")
    }
        ...
        if errors:
            ...
            for i, error_list in enumerate(errors[:3], 1):  # Show first 3 errors
                if isinstance(error_list, list) and error_list:
                    table.add_row(f"  Error {i}", error_list[0][:60] + "..." if len(error_list[0]) > 60 else error_list[0])
        else:
...
async def verify_mongodb_direct(
    ...
    if HAS_RICH:
        ...
        console.print("[dim]   Query: db.chunks.countDocuments({document_id: ObjectId('...')})[/dim]")
    else:
...
async def main():
    ...
    try:
        ...
        document_ids = ingestion_result.get("document_ids", [])
        ...
        for query, search_type in test_queries:
            try:
                search_result = await search_knowledge_base(query, search_type=search_type, match_count=3)
        ...
        if HAS_RICH:
            ...
            console.print("\n[bold]Summary:[/bold]")
            console.print(f"  • Documents processed: {ingestion_result.get('documents_processed', 0)}")
