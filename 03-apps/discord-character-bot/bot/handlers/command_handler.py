...
class CharacterSelectView(View):
    ...
    async def add_button(self, interaction: discord.Interaction, button: Button):
        ...
        if self.characters:
            ...
            try:
                result = await self.api_client.add_character(
                    str(self.channel_id),
                    character["id"]
                )
                if result.get("success"):
                    await interaction.followup.send(
                        f"Character '{character['name']}' added to channel!",
                        ephemeral=True
                    )
                else:
                    await interaction.followup.send(
                ...
                await interaction.followup.send(
                    f"Error adding character: {str(e)}",
                    ephemeral=True
                )


async def handle_add_character(
    ...
    try:
        ...
        if result.get("success"):
            ...
            await interaction.followup.send(
                f"❌ {result.get('message', 'Failed to add character')}",
                ephemeral=True
            )
    except Exception as e:
...
async def handle_remove_character(
    ...
    try:
        ...
        if result.get("success"):
            await interaction.followup.send(
                f"✅ {result.get('message', 'Character removed')}",
                ephemeral=True
            )
        else:
            await interaction.followup.send(
                f"❌ {result.get('message', 'Failed to remove character')}",
                ephemeral=True
            )
    except Exception as e:
        logger.exception(f"Error in remove_character: {e}")
        await interaction.followup.send(
            f"❌ Error removing character: {str(e)}",
            ephemeral=True
        )


async def handle_list_characters(
    interaction: discord.Interaction,
    api_client: APIClient
):
    ...
    try:
        ...
        for char in characters:
            ...
            embed.add_field(
                name=name,
                value=byline or "No description",
                inline=False
            )
        ...
        await interaction.followup.send(
            f"❌ Error listing characters: {str(e)}",
            ephemeral=True
        )
...
async def handle_clear_history(
    ...
    try:
        ...
        if result.get("success"):
            await interaction.followup.send(
                f"✅ {result.get('message', 'History cleared')}",
                ephemeral=True
            )
        else:
            await interaction.followup.send(
                f"❌ {result.get('message', 'Failed to clear history')}",
                ephemeral=True
            )
    except Exception as e:
...
async def handle_query_knowledge_store(
    ...
    try:
        ...
        await interaction.followup.send(
            f"Error querying knowledge store: {str(e)}",
            ephemeral=True
        )
