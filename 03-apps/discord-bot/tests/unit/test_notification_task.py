...
async def test_notification_task_process_asset_with_faces(
    ...
    with patch.object(mock_notification_task.immich_client, 'get_asset_faces', new_callable=AsyncMock) as mock_faces:
        mock_faces.return_value = sample_immich_faces
        
        with patch.object(mock_notification_task.immich_client, 'get_asset_thumbnail', new_callable=AsyncMock) as mock_thumb:
...
async def test_notification_task_duplicate_prevention(
    ...
    with patch.object(mock_notification_task.immich_client, 'get_asset_faces', new_callable=AsyncMock) as mock_faces:
        ...
        with patch.object(mock_notification_task.immich_client, 'get_asset_thumbnail', new_callable=AsyncMock):
            mock_user = AsyncMock()
            mock_user.send = AsyncMock()
            mock_notification_task.client.fetch_user = AsyncMock(return_value=mock_user)
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_notification_task_poll_loop_initialization(
    mock_notification_task, test_database
):
    ...
    with patch.object(mock_notification_task.immich_client, 'list_new_assets', new_callable=AsyncMock) as mock_list:
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_notification_task_poll_loop_updates_timestamp(
    mock_notification_task, test_database
):
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_notification_task_handles_errors_gracefully(
    mock_notification_task, test_database
):
