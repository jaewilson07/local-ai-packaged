...
async def test_full_notification_flow(
    ...
    with patch.object(immich_client, "get_asset_faces", new_callable=AsyncMock) as mock_faces:
        ...
        with patch.object(
            immich_client, "get_asset_thumbnail", new_callable=AsyncMock
        ) as mock_thumb:
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_notification_flow_no_faces(test_database, mock_discord_client, sample_immich_asset):
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_notification_flow_timestamp_tracking(test_database, mock_discord_client):
