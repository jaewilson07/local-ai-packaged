...
def validate_config():
    ...
    print(f"  DISCORD_UPLOAD_CHANNEL_ID: {'✓ Set' if config.DISCORD_UPLOAD_CHANNEL_ID else '✗ Missing'}")
...
async def validate_immich():
    ...
    try:
        ...
        print(f"✅ Immich connection successful!")
        print(f"  Server: {config.IMMICH_SERVER_URL}")
        print(f"  Found {len(people)} people in database")
        return True
        
    except Exception as e:
        print(f"❌ Immich connection failed: {type(e).__name__}: {str(e)}")
        print()
...
async def validate_discord():
    ...
    try:
        ...
        try:
            ...
            print(f"❌ Discord connection failed: {type(e).__name__}: {str(e)}")
...
async def validate_database():
    ...
    try:
        ...
        print(f"❌ Database validation failed: {type(e).__name__}: {str(e)}")
        return False
...
async def validate_mcp():
    ...
    try:
        ...
        print(f"❌ MCP server validation failed: {type(e).__name__}: {str(e)}")
        return False
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate Discord bot configuration and connectivity")
    parser.add_argument(
        "command",
