...
Infisical requires two encryption keys. You can generate them automatically or manually.

### Option A: Automatic Generation (Recommended)

Use the password generator script to automatically generate and add the keys to your `.env` file:

```bash
python utils/setup/env/generate_passwords.py
```
...
If you prefer to generate them manually:
...
### Optional: Configure Google OAuth/SSO
...
1. **Create OAuth2 Application in Google Cloud Platform:**
   ...
   - Add authorized redirect URIs:
     - For local: `http://localhost:8010/api/v1/auth/oauth2/google/callback`
...
4. **Enable in Infisical UI:**
   - After restarting, go to **Settings** → **SSO** in Infisical UI
   - Enable Google SSO
   - Users can now sign in with their Google accounts

**Note:** The `SITE_URL` environment variable must match your Infisical URL for OAuth redirects to work correctly.
