...
def sync_to_infisical(env_file: Path):
    ...
    try:
        subprocess.run(
            ['python3', str(sync_script), '--env-file', str(env_file)],
            check=True
        )
...
Examples:
  ...
  python3 setup-google-sso.py --client-secret-file /mnt/c/Users/username/Downloads/client_secret.json
    ...
    parser.add_argument(
        '--client-secret-file',
        required=True,
        help='Path to Google OAuth2 client secret JSON file'
    )
    parser.add_argument(
        '--site-url',
    ...
    parser.add_argument(
        '--env-file',
        help='Path to .env file (default: auto-detect from repo root)'
    )
