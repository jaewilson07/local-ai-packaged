...
def run_command(cmd, check=True, capture_output=False):
    ...
    result = subprocess.run(cmd, text=True, check=False, capture_output=capture_output)
...
def identify_organization_tables(tables, include_users=False):
    ...
    for table in tables:
        ...
        if any(pattern in table_lower for pattern in user_patterns):
            if not include_users and (table_lower in {"users", "user"}):
                continue
...
def create_database_backup(db_user, db_name, db_password, backup_path):
    ...
    if result.returncode == 0:
        ...
        if run_command(copy_cmd, check=False).returncode == 0:
            run_command(
                ["docker", "exec", "infisical-db", "rm", container_backup_path], check=False
            )
...
def flush_redis_cache(dry_run=False):
    ...
    if (
        run_command(
            ["docker", "exec", "infisical-redis", "redis-cli", "FLUSHALL"], check=False
        ).returncode
        == 0
    ):
...
def main():
    ...
    parser_data.add_argument(
        "--backup", action="store_true", help="Create DB backup before deletion"
    )
    parser_data.add_argument(
        "--include-users", action="store_true", help="Also delete user accounts"
    )
