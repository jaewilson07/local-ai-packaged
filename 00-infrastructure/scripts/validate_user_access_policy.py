...
def check_email_access_to_app(email: str, app: Dict, policies: List[Dict]) -> Tuple[bool, List[str]]:
...
def main():
    ...
    parser.add_argument(
        "email",
        help="Email address to check access for"
    )
    parser.add_argument(
        "--account-id",
    ...
    if not account_id:
        print("[ERROR] Account ID is required. Set CLOUDFLARE_ACCOUNT_ID in .env or use --account-id")
        return
    ...
    for app in apps:
        ...
        if policy_response.status_code != 200:
            if args.verbose:
                print(f"⚠️  {app_name} ({app_domain}): Could not fetch policies (HTTP {policy_response.status_code})")
        ...
        if has_access:
            ...
            if args.verbose:
                print(f"❌ {app_name}")
                print(f"   Domain: {app_domain}")
                print(f"   Reason: No matching policies")
            ...
            if tunnel_data.get("success"):
                ...
                for rule in ingress:
                    ...
                    if access_enabled:
                        ...
                        if matching_app:
                            print(f"   ✅ Matches accessible application: {matching_app.get('name')}")
                    else:
                        print(f"   Access: Disabled")
                    print()
            else:
