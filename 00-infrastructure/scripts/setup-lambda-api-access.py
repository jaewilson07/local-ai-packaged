...
def create_lambda_api_access_application(headers, account_id):
    ...
    if response.status_code == 200:
        ...
        for app in apps:
            if app.get("domain") == FULL_DOMAIN:
                app_id = app["id"]
                print(
                    f"[OK] Lambda API Access application already exists: {FULL_DOMAIN} (ID: {app_id})"
                )
                return app_id
    ...
    allowed_emails = (
        [e.strip() for e in allowed_emails_str.split(",") if e.strip()]
    ...
    app_data = {
        ...
        "policies": [
            {"name": "Lambda API Access Policy", "decision": "allow", "include": include_rules}
        ],
    }
...
def link_access_to_tunnel_route(headers, account_id, app_id):
    ...
    print("   4. Click Edit")
    print("   5. Under Access, select 'Lambda API'")
    return False
