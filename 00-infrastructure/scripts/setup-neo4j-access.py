...
def create_neo4j_access_application(headers, account_id):
    ...
    if response.status_code in [200, 201]:
        ...
        if result.get("success"):
            ...
            if errors:
                print(f"[ERROR] Failed to create application: {errors[0].get('message', 'Unknown error')}")
            return None
    else:
...
def link_access_to_tunnel_route(headers, account_id, app_id):
    ...
    if update_response.status_code == 200:
        ...
        if result.get("success"):
            ...
            errors = result.get("errors", [])
            if errors:
                print(f"[ERROR] Failed to link Access application: {errors[0].get('message', 'Unknown error')}")
            return False
    else:
        print(f"[WARNING] Failed to link Access application via API: HTTP {update_response.status_code}")
        print(f"   Link manually in dashboard:")
        ...
        print(f"   4. Under Access, select 'Neo4j'")
        return False
